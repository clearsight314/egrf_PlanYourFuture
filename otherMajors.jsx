...
const otherDegreeAuditUI = () => {
    ...
    useEffect(() => {
        fetch("./all_majors_audit.json").then(r => r.json()).then(setAllAuditData).catch(e => { console.error(e); setLoadError("Failed to load audit data: " + e.message); });
        ...
        // Seed state from whatever the dropdowns already show on mount
        const readVal = id => document.getElementById(id)?.value || "";
        setSelectedMajorName(readVal("sel-major"));
        setSelectedMajor2Name(readVal("sel-major2"));
        setSelectedMinorName(readVal("sel-minor"));
        setSelectedMinor2Name(readVal("sel-minor2"));
    ...
    if (loadError) return <div style={{ padding: "20px", color: "#dc2626" }}>{loadError}</div>;
...
window.otherDegreeAuditUI = otherDegreeAuditUI;
