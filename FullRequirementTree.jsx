...
const SubGraph = ({ graphData, allowedCourseIds, reqType, anyCount }) => {
    ...
    return (
        ...
        >
            ...
            >
                Course Map (
                {reqType === "ANY" ? `Fulfill Any ${anyCount}` : "Fulfill All"})
            </div>
            <div
...
const RequirementNode = ({
    ...
    const hasCourses = nodeCourses.length > 0;
    ...
    return (
        <li>
            ...
            >
                ...
                <div
                    style={{ cursor: "pointer", width: "100%", padding: "4px" }}
...
const FullRequirementTree = () => {
    ...
    useEffect(() => {
        fetch("./uva_cs_audit_cleaned.json")
            ...
            .catch((err) => console.error("Error loading Audit JSON:", err));

        fetch("./cs_course_graph.json")
            .then((res) => res.json())
            .then((data) => setGraphData(data))
            .catch((err) => console.error("Error loading Graph JSON:", err));
    }, []);
